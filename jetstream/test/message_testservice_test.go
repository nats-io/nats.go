...
func TestAckVariants(t *testing.T) {
	// setup creates a fresh JS server with stream "foo" / consumer "cons"
	// inside the given callback. Each subtest gets its own server so
	// per-subtest state (the SubscribeSync on msg.Reply) is isolated.
	setup := func(t *testing.T, fn func(ctx context.Context, t *testing.T, nc *nats.Conn, js jetstream.JetStream, c jetstream.Consumer)) {
		t.Helper()
	...
	t.Run("standard ack", func(t *testing.T) {
		setup(t, func(ctx context.Context, t *testing.T, nc *nats.Conn, js jetstream.JetStream, c jetstream.Consumer) {
	...
	t.Run("ack twice", func(t *testing.T) {
		setup(t, func(ctx context.Context, t *testing.T, _ *nats.Conn, js jetstream.JetStream, c jetstream.Consumer) {
			if _, err := js.Publish(ctx, "FOO.1", []byte("msg")); err != nil {
				t.Fatalf("Unexpected error: %v", err)
			}
			msgs, err := c.Fetch(1)
