...
func TestListObjectStores(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			withJSServer(t, func(t *testing.T, _ *nats.Conn, js jetstream.JetStream) {
				ctx := newTesterCtx(t, 60*time.Second)
...
func TestObjectStoreGetObjectContextTimeout(t *testing.T) {
	withJSServer(t, func(t *testing.T, _ *nats.Conn, js jetstream.JetStream) {
		setupCtx := newTesterCtx(t, 10*time.Second)
		obs, err := js.CreateObjectStore(setupCtx, jetstream.ObjectStoreConfig{Bucket: "OBJS"})
		expectOk(t, err)
		...
		_, err = obs.PutBytes(setupCtx, "blob", blob)
		expectOk(t, err)

		ctx := newTesterCtx(t, 10*time.Millisecond)
