...
func TestTesterJSCluster(t *testing.T) {
	withTesterJSCluster(t, 3, func(t *testing.T, nc *nats.Conn, js jetstream.JetStream, inst *testservice.Instance) {
		...
		if len(inst.Servers) != 3 {
			t.Fatalf("expected 3 servers in cluster, got %d", len(inst.Servers))
		}

		s, err := js.CreateStream(ctx, jetstream.StreamConfig{
			Name:     "POC2",
			Subjects: []string{"poc2.*"},
			Replicas: 3,
		})
		if err != nil {
		...
		for i := range 10 {
			if _, err := js.Publish(ctx, "poc2.x", []byte{byte('0' + i)}); err != nil {
		...
		if got != 10 {
			t.Fatalf("expected 10 fetched msgs, got %d", got)
		}
	})
}
