...
func TestLeakingGoRoutinesOnFailedConnect(t *testing.T) {
	...
	// Use a port that is guaranteed to fail (privileged + no listener).
	// Cannot use nats.DefaultURL here because in the host-side dev workflow
	// the tester publishes its own management server on localhost:4222.
...
func TestConnectedServer(t *testing.T) {
	withServerInstance(t, func(t *testing.T, nc *nats.Conn, inst *testservice.Instance) {
		wantURL := inst.Servers[0].URL
		if u := nc.ConnectedUrl(); u == "" || u != wantURL {
			t.Fatalf("Unexpected connected URL of %s (want %s)", u, wantURL)
		}
		if id := nc.ConnectedServerId(); id == "" {
		...
		if u := nc.ConnectedUrl(); u != "" {
			t.Fatalf("Expected a nil connected URL, got %s", u)
		}
		if id := nc.ConnectedServerId(); id != "" {
...
accounts: {
	...
	nc := dialInstance(t, inst, nats.UserInfo("sys", "pass"))
	...
	nc2 := dialInstance(t, inst, nats.UserInfo("app", "pass"))
...
func TestBadOptionTimeoutConnect(t *testing.T) {
	withServerInstance(t, func(t *testing.T, _ *nats.Conn, inst *testservice.Instance) {
		...
		if !strings.Contains(err.Error(), "invalid") {
			t.Fatalf("Expected a ErrNoServers error: Got %v", err)
		}
	})
}
...
func TestAsyncSubscribeRoutineLeakOnClose(t *testing.T) {
	withServerInstance(t, func(t *testing.T, _ *nats.Conn, inst *testservice.Instance) {
		...
		// Take the base BEFORE the connection is created so the connection's
		// goroutines count toward the leak delta.
		base := getStableNumGoroutine(t)
...
func TestFlush(t *testing.T) {
	withServer(t, func(t *testing.T, nc *nats.Conn) {
		...
		if err := nc.Flush(); err != nil {
			t.Fatalf("Received error from flush: %s", err)
		}
		if nb, _ := nc.Buffered(); nb > 0 {
			t.Fatalf("Outbound buffer not empty: %d bytes", nb)
		}
...
func TestRequestTimeout(t *testing.T) {
	withServer(t, func(t *testing.T, nc *nats.Conn) {
		// We need a responder by default otherwise we get a no responders error.
...
func TestBasicNoRespondersSupport(t *testing.T) {
	withServerInstance(t, func(t *testing.T, nc *nats.Conn, inst *testservice.Instance) {
		...
		// Old request style (separate connection)
		ncOld := dialInstance(t, inst, nats.UseOldRequestStyle())
		if m, err := ncOld.Request("foo", nil, time.Second); err != nats.ErrNoResponders {
			t.Fatalf("Expected a no responders error and nil msg, got m:%+v and err: %v", m, err)
		}
		ctx2, cancel2 := context.WithTimeout(context.Background(), time.Second)
		defer cancel2()
		if m, err := ncOld.RequestWithContext(ctx2, "foo", nil); err != nats.ErrNoResponders {
			t.Fatalf("Expected a no responders error and nil msg, got m:%+v and err: %v", m, err)
		}

		// SubscribeSync style
		inbox := nats.NewInbox()
		sub, err := ncOld.SubscribeSync(inbox)
		if err != nil {
			t.Fatal(err)
		}
		if err := ncOld.PublishRequest("foo", inbox, nil); err != nil {
			t.Fatal(err)
		}
...
func TestStats(t *testing.T) {
	withServer(t, func(t *testing.T, nc *nats.Conn) {
		...
		if nc.OutMsgs != uint64(iter) {
			t.Fatalf("Not properly tracking OutMsgs: received %d, wanted %d", nc.OutMsgs, iter)
		}
		obb := uint64(iter * len(data))
		if nc.OutBytes != obb {
			t.Fatalf("Not properly tracking OutBytes: received %d, wanted %d", nc.OutBytes, obb)
		}
		...
		if nc.InMsgs != uint64(2*iter) {
			t.Fatalf("Not properly tracking InMsgs: received %d, wanted %d", nc.InMsgs, 2*iter)
		}

		ibb := 2 * obb
		if nc.InBytes != ibb {
			t.Fatalf("Not properly tracking InBytes: received %d, wanted %d", nc.InBytes, ibb)
		}
	})
}
...
func TestRaceSafeStats(t *testing.T) {
	withServer(t, func(t *testing.T, nc *nats.Conn) {
		...
		if stats.OutMsgs != uint64(1) {
			t.Fatalf("Not properly tracking OutMsgs: received %d, wanted %d", nc.OutMsgs, 1)
		}
	})
}
...
func TestBadSubject(t *testing.T) {
	withServer(t, func(t *testing.T, nc *nats.Conn) {
		if err := nc.Publish("", []byte("Hello World")); !errors.Is(err, nats.ErrBadSubject) {
			t.Fatalf("Expected a ErrBadSubject error: Got %v", err)
		}
		if err := nc.Publish("foo bar", []byte("Hello World")); !errors.Is(err, nats.ErrBadSubject) {
			t.Fatalf("Expected a ErrBadSubject error: Got %v", err)
		}
		msg := &nats.Msg{Subject: "foo", Reply: "bar baz", Data: []byte("Hello World")}
		if err := nc.PublishMsg(msg); !errors.Is(err, nats.ErrBadSubject) {
			t.Fatalf("Expected a ErrBadSubject error: Got %v", err)
		}
	})
}
...
func TestOptions(t *testing.T) {
	withServerInstance(t, func(t *testing.T, _ *nats.Conn, inst *testservice.Instance) {
		...
		// Stop + Start the only server on the same port to trigger reconnect.
		...
		// Second connection with NoReconnect — stopping the server should NOT
		// trigger a reconnect callback.
		nc2, err := nats.Connect(inst.Servers[0].URL, nats.NoReconnect())
		if err != nil {
			t.Fatalf("Failed to connect: %v", err)
		}
		t.Cleanup(nc2.Close)

		nc2.SetReconnectHandler(func(_ *nats.Conn) { rch <- true })
		nc2.SetClosedHandler(func(_ *nats.Conn) { cch <- true })
...
func TestNilConnection(t *testing.T) {
	...
	if err := nc.Publish("foo", data); err == nil || err != nats.ErrInvalidConnection {
		t.Fatalf("Expected ErrInvalidConnection error, got %v", err)
	}
	if err := nc.PublishMsg(nil); err == nil || err != nats.ErrInvalidMsg {
		t.Fatalf("Expected ErrInvalidMsg error, got %v", err)
	}
	...
	if sub.Type() != nats.NilSubscription {
		t.Fatalf("Got wrong type for nil subscription, %v", sub.Type())
	}
	if sub.IsValid() {
		t.Fatalf("Expected IsValid() to return false")
	}
	if err := sub.Unsubscribe(); err == nil || err != nats.ErrBadSubscription {
		t.Fatalf("Expected Unsubscribe to return proper error, got %v", err)
	}
	if err := sub.AutoUnsubscribe(1); err == nil || err != nats.ErrBadSubscription {
		t.Fatalf("Expected ErrBadSubscription error, got %v", err)
	}
