...
func TestExpiredAuthentication(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"expired users credentials", nats.AUTHENTICATION_EXPIRED_ERR, nats.ErrAuthExpired, false},
		{"revoked users credentials", nats.AUTHENTICATION_REVOKED_ERR, nats.ErrAuthRevoked, false},
		{"expired account", nats.ACCOUNT_AUTHENTICATION_EXPIRED_ERR, nats.ErrAccountAuthExpired, false},
		{"expired users credentials", nats.AUTHENTICATION_EXPIRED_ERR, nats.ErrAuthExpired, true},
		{"revoked users credentials", nats.AUTHENTICATION_REVOKED_ERR, nats.ErrAuthRevoked, true},
		{"expired account", nats.ACCOUNT_AUTHENTICATION_EXPIRED_ERR, nats.ErrAccountAuthExpired, true},
	} {
			...
			go func() {
				...
				for {
					...
					if connect++; connect == 1 {
						// conn.Write([]byte(fmt.Sprintf("%s%s", nats._PONG_OP_, _CRLF_)))
			...
			if test.ignoreAbort {
				...
				for i := 0; i < 4; i++ {
					...
					case e := <-errCh:
						...
						} else if i > 0 && e != nats.ErrAuthorization {
							t.Fatalf("Expected error %q, got %q", nats.ErrAuthorization, e)
						}
					case <-time.After(time.Second):
						if i == 0 {
							t.Fatalf("Missing %q error", test.expectedErr)
						} else {
							t.Fatalf("Missing %q error", nats.ErrAuthorization)
						}
					}
				}
				return
