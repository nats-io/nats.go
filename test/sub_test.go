...
func TestMaxSubscriptionsExceeded(t *testing.T) {
	c := newTester(t)
	inst := c.CreateServer(t, false, testservice.WithTopLevel("max_subscriptions: 5"))
	t.Cleanup(func() { inst.Destroy(t) })

	ch := make(chan error)
	nc, err := nats.Connect(inst.Servers[0].URL, nats.ErrorHandler(func(c *nats.Conn, s *nats.Subscription, err error) {
		ch <- err
	}))
	if err != nil {
...
const denyFooSubscribeAuthSnippet = `
authorization: {
    users = [
        {
            user: test
...
func TestSubscribeSyncPermissionError(t *testing.T) {
	c := newTester(t)
	inst := c.CreateServer(t, false, testservice.WithAuthorization(denyFooSubscribeAuthSnippet))
	t.Cleanup(func() { inst.Destroy(t) })
	...
	t.Run("PermissionErrOnSubscribe disabled", func(t *testing.T) {
		nc, err := nats.Connect(inst.Servers[0].URL, nats.UserInfo("test", "test"))
		if err != nil {
			t.Fatalf("Error on connect: %v", err)
		}
		defer nc.Close()
